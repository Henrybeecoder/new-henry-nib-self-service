...
function OpenAccount(props) {
    ...
    const { accountOpeningStep } = useSelector((state) => state.accountOpeningData)


    // const [selectedAccountType, setSelectedAccountType] = useState('')
    const [accountType, setAccountType] = useState(localStorage.getItem('altFinanceAccountType'))
                ...
                <div className="col-md-7 right_col">
                            ...
                            <nav className="mb-4">
                                <ol className="breadcrumb bg-white float-right">
                                    <li className="breadcrumb-item"> <a href="#">
                                        <span><img className="pr-1" src={group1} alt="" /></span> Bvn Validation
                                        &nbsp;<span><img src={rightArrow} alt="" /></span>
                                    </a>

                                    </li>
                                    <li className="breadcrumb-item"> <a href="#">
                                        <span><img className="pr-1" src={no2} alt="" /></span> Update details
                                        &nbsp;<span><img src={rightArrow} alt="" /></span>
                                    </a>
                                    </li>
                                    <li className="breadcrumb-item"> <a href="#">
                                        <span><img className="pr-1" src={no3} alt="" /></span> Document Upload
                                    </a>
                                    </li>
                                </ol>
                            </nav>
                    ...
                    <div className="row">
                        <div className="col-md-11 card validation-card pb-4">
                            {accountOpeningStep === 'bvn-validation' ? <BvnValidationDialog /> : ''}

                            {accountOpeningStep === 'otp' ? <Otp /> : ''}

                            {accountOpeningStep === 'swift-savings'  ? <UpdateDetailSwiftSavingsDialog /> : ''}
                            {accountOpeningStep === 'update-detail' && accountType === 'savings' ? <UpdateDetailSavingsDialog /> : ''}
                            {accountOpeningStep === 'update-detail' && accountType === 'minor savings' ? <UpdateDetailMinorSavingsDialog /> : ''}
                            {accountOpeningStep === 'update-detail' && accountType === 'joint savings' ? <UpdateDetailJointSavingsDialog /> : ''}
                            {accountOpeningStep === 'update-detail' && accountType === 'domiciliary savings' ? <UpdateDetailDomiciliarySavingsDialog /> : ''}
                            {accountOpeningStep === 'update-detail' && accountType === 'current' ? <UpdateDetailCurrentDialog /> : ''}
                            {accountOpeningStep === 'update-detail' && accountType === 'joint current' ? <UpdateDetailJointCurrentDialog /> : ''}
                            {accountOpeningStep === 'update-detail' && accountType === 'domiciliary current' ? <UpdateDetailDomiciliaryCurrentDialog /> : ''}




                            {accountOpeningStep === 'document-upload' ? <DocumentUploadDialog /> : ''}
                            {accountOpeningStep === 'account-opening-success' ? <AccountOpeningSuccessDialog /> : ''}
